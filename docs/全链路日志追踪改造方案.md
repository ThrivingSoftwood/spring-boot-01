...
<Property name="CONSOLE_PATTERN">
    ...
    %d{HH:mm:ss.SSS} %highlight{%-5level} [%style{%X{traceId}}{cyan}][%style{%X{spanId}}{blue}] %t : %msg%n
</Property>
```
...
```text
2026-01-28 18:52:01 INFO  [1234567890123456][main] http-nio-8080-exec-1 : === 接收到前端请求 ===
```
...
```text
2026-01-28 18:52:02 INFO  [1234567890123456][sub-98765] virtual-1 : 远程配置获取成功
```
...
```text
2026-01-28 18:52:03 ERROR [1234567890123456][sub-11223] pool-1-thread-1 : ❌ 异步任务执行异常! 
java.lang.RuntimeException: 模拟异常
    at thriving.softwood...
```
...
1. **TraceID 格式**：目前使用的是 Snowflake ID（纯数字）。如果需要与外部系统（如 OpenTelemetry / Jaeger）对接，建议修改
   `TraceUtil` 生成符合 **W3C Trace Context 标准** 的 ID（32位十六进制字符串）。
