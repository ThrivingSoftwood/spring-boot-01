...
@Component
public abstract class AbstractDataPermissionSqlBuilder {

    // 🌟 注入所有实现了 ResourceExtractor 接口的 Bean
    protected final List<ResourceExtractor> extractors;

    public AbstractDataPermissionSqlBuilder(ObjectProvider<ResourceExtractor> extractorProvider) {
    ...
    protected abstract String buildFragmentForRule(DataRuleDTO rule);

    protected static @NonNull StringBuilder getSqlFragmentBuilder(DataRuleDTO rule) {
